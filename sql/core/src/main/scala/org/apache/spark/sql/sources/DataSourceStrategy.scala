...
private[sql] object DataSourceStrategy extends Strategy {
  def apply(plan: LogicalPlan): Seq[execution.SparkPlan] = plan match {
    ...
    case l @ LogicalRelation(t: TableScan) =>
      execution.PhysicalRDD(l.output, t.buildScan().setName(l.relation.tableName.orNull)) :: Nil
    ...
    if (projectList.map(_.toAttribute) == projectList &&
      ...
      val scan =
        execution.PhysicalRDD(
          ...
          scanBuilder(requestedColumns, pushedFilters).setName(relation.relation.tableName.orNull))
