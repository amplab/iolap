...
trait HiveTypeCoercion {
  ...
  object FunctionArgumentConversion extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan = plan transformAllExpressions {
      ...
      case If(p, t, f) if t.dataType != f.dataType =>
        val tt = t.dataType
        val ft = f.dataType
        val rt = findTightestCommonType(tt, ft)
        rt match {
          case Some(finaldt) => If(p, Cast(t, finaldt), Cast(f, finaldt))
          case None => sys.error(s"Could not determine return type of If for $tt, $ft")
        }
    }
  }
