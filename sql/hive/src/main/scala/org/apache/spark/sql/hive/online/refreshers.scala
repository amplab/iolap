...
import org.apache.spark.storage.LazyBlockId
...
case class UpdatesHashMap(opId: OpId, numPartitions: Int, batchId: Int) {
  ...
  def apply(key: Row): Row = {
    ...
    if (maps(index) == null) {
      ...
      numActiveMaps += 1
      maps(index) =
        SparkEnv.get.blockManager.getSingle(lazyBlockId) match {
      ...
      if (numActiveMaps > 1) {
        (0 until numPartitions).foreach { idx =>
          if (maps(idx) == null) future {
            SparkEnv.get.blockManager.getSingle(LazyBlockId(opId.id, batchId, idx))
          }
        }
      }
    }
      ...
      dependencies.foldLeft(iterator) { (iter, dependency) =>
        dependency._2._2 match {
          case Seq() =>
            ...
            val row = SparkEnv.get.blockManager.getSingle(lazyBlockId) match {
