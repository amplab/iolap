...
object PushUpResample extends Rule[SparkPlan] {
  override def apply(plan: SparkPlan): SparkPlan = plan.transformUp {
    ...
    case Sort(sortOrder, global, ResamplePlaceholder(b, child)) =>
      ResamplePlaceholder(b, Sort(sortOrder, global, child))
  }
}
                ...
                case alias@Alias(expr, name) if isAggregate(expr) =>
                  val newExpr = expr.transformUp {
                    case sum: Sum =>
                      ...
                      If(ByteNonZero(aBtCnt.child), dSum, Literal(null, dSum.dataType))
...
object OnlinePlannerUtil {
  ...
  val simplifyCast: PartialFunction[Expression, Expression] = {
    case expr@Cast(_: Literal, dataType) => Literal(expr.eval(EmptyRow), dataType)
