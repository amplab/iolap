...
class OnlineDataFrame(dataFrame: DataFrame) extends org.apache.spark.Logging {
  private[this] val prepareForExecution = dataFrame.sqlContext.prepareForExecution
  ...
  lazy val executedPlan = prepareForExecution.execute(
    olPlanner.execute(dataFrame.queryExecution.sparkPlan))
